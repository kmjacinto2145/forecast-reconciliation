...
lc_decile_sex_forecasts = list()

i = 1
for (model in lc_decile_sex) {
    forecasts = forecast(model, h = H)
    lc_decile_sex_forecasts[[i]] = forecasts$rates
    i = i + 1
}

names(lc_decile_sex_forecasts) = names(lc_decile_sex)
...
# saveRDS(St_list, "england_models 60-89/St_list.rds")

#Shape base forecasts
B_forecasts = list()
for (h in 1:H) {
    for (age in 60:89) {
        tmp_vec = sapply(lc_decile_sex_forecasts, function(m) {
            return(m[as.character(age),h])
        })
...
saveRDS(R_forecasts, "england_models 60-89/forecasts_bu.rds")
